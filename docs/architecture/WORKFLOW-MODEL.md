...
版本：1.1；日期：2026-09-05。需求：FR02/03/05/06/08/09/18–20。

## 1. 工作流控制什么

外层状态图约束阶段、允许能力、状态转移、证据门禁和终止；模型在这些边界内理解问题、提出假设和下一动作。通用模型能力贯穿各节点，不是固定排在“外部知识之后”的一个步骤。

先理解目标、子问题和缺失前提，再路由；低风险稳定通用解释可显式直答，产品事实默认先检索内部知识。结果充足直接回答；不足时根据所缺事实选择官方检索、产品 API/MCP、已注册诊断，或向用户/工程师索取资料。优先级是同等合规、适用候选中的偏好，不能迫使每个任务跑遍全部工具。

~~~mermaid
flowchart TD
    I["输入 / 关联 Case / 权限检查"] --> F["理解问题 / 子问题与缺口"]
    F -->|"产品事实"| K["内部检索"]
    F -->|"低风险稳定通用解释"| A
    F -->|"关键前提缺失"| H
    K --> E["证据充分性与冲突判断"]
...
initial_stage: understand_question
capabilities:
  - id: knowledge.internal
    priority: 100
    when: product_fact_or_evidence_needed
  - id: web.official
    priority: 90
...
7. 最终生成后检查子问题覆盖、命题支持、版本/前提、建议可行动性与当前输出权限；不通过最多一次修正。缺事实回调查补证，重生成不重置 Run 预算和无进展限制。仍失败仅发布通过检查的受限阶段答复/人工请求。

理解/检索恢复/缺口选动作/答案检查的语义详见 [QA Core](QA-CORE.md)。完整答复与阶段答复分开；有关键待办不能仅因发布了消息而 RESOLVED。查询改写默认每缺口最多两次，与下述 Provider 重试和无进展规则共同计入总预算。
